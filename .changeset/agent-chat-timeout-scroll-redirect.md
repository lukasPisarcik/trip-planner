...
- **Run timer.** Each settled reply shows a persisted "Ran for `m:ss`" (survives reload), and a pink **"Chat total"** pill above the composer tracks the whole chat's cumulative working time — the sum of every turn's duration — ticking live while a turn streams.
